...
local render_numbers = ya.sync(function(_, mode)
	...
	Current.redraw = function(self)
		...
		-- Reserve a fixed-width column for the numbers, e.g. "123 " -> 4 cols
		local num_width = #tostring(math.max(#files, 1)) + 1

		local areas = ui.Layout()
			:direction(ui.Layout.HORIZONTAL)
			:constraints({
				ui.Constraint.Length(num_width),
				ui.Constraint.Fill(1),
			})
			:split(self._area)

		local num_area, list_area = areas[1], areas[2]

		local entities, linemodes, numbers = {}, {}, {}
		for i, f in ipairs(files) do
			linemodes[#linemodes + 1] = Linemode:new(f):redraw()
			numbers[#numbers + 1] = ui.Line({ Entity:number(i, #self._folder.files, f, hovered_index) })

			local entity = Entity:new(f)
			entities[#entities + 1] = entity:redraw():style(entity:style())
		end

		return {
			-- unmodified stock rendering, so title/selection highlight is exactly as before
			ui.List(entities):area(list_area),
			ui.Text(linemodes):area(list_area):align(ui.Align.RIGHT),
			-- numbers drawn in their own reserved column, never touched by row highlight
			ui.Text(numbers):area(num_area),
		}
	end
end)
